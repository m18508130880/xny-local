		...
		while(true)
		{
			try
			{
		        ...
		        for(int i = 0; i<sendList.size(); i++)
		        {
			        ...
			        SetSendMsg(CommUtil.StrBRightFillSpace(this.m_ID, 20) + 
			          CommUtil.StrBRightFillSpace("", 20) + "0000" + 
			          4001 + 
			          CommUtil.StrBRightFillSpace(StationBean.getCPM_Id(), 10) + 
			          CommUtil.StrBRightFillSpace(StationBean.getCPM_Name(), 30) + 
			          CommUtil.StrBRightFillSpace(StationBean.getCPM_Brief(), 20) + 
			          CommUtil.StrBRightFillSpace(StationBean.getCPM_Status(), 1) + 
			          CommUtil.StrBRightFillSpace(StationBean.getCPM_OnOff(), 1) + 
			          CommUtil.StrBRightFillSpace(StationBean.getCPM_Type(), 4) + 
			          CommUtil.StrBRightFillSpace(StationBean.getCPM_Time(), 10) + 
			          CommUtil.StrBRightFillSpace(StationBean.getLink_Url(), 20) + 
			          CommUtil.StrBRightFillSpace(StationBean.getLink_Port(), 6) + 
			          CommUtil.StrBRightFillSpace(StationBean.getLink_Id(), 20) + 
			          CommUtil.StrBRightFillSpace(StationBean.getLink_Pwd(), 6) + 
			          StationBean.getGis_Sign() + 
			          CommUtil.StrBRightFillSpace(StationBean.getLongitude(), 10) + 
