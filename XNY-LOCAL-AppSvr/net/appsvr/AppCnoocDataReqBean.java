...
{	
	...
	public int execRequest()
	{
		...
		{	
			...
			{
				//数据解析
				JiNanXNYData27Bean Data27Bean = new JiNanXNYData27Bean();				
				byte[] _Value1 = new byte[14];
				byte[] _Value2 = new byte[20];
				byte[] _Value5 = new byte[16];
				byte[] _Value6 = new byte[20];
				...
				DataInputStream DinStream = new DataInputStream(new ByteArrayInputStream(Dev_CData));
				DinStream.read(_Value1, 0, 14);
				Data27Bean.setCTime(new String(_Value1));
				DinStream.read(_Value2, 0, 20);
				Data27Bean.setOrder_Id(new String(_Value2));
				Data27Bean.setOrder_Value(CommUtil.converseInt(DinStream.readInt()));
				Data27Bean.setValue(CommUtil.converseInt(DinStream.readInt()));
				DinStream.read(_Value5, 0, 16);
				Data27Bean.setCar_Id(new String(_Value5));
				DinStream.read(_Value6, 0, 20);
				Data27Bean.setCar_Owner(new String(_Value6));
				DinStream.read(_Value7, 0, 20);
				Data27Bean.setWorker(new String(_Value7));
				DinStream.read(_Value8, 0, 4);
				Data27Bean.setOil_CType(new String(_Value8));
				DinStream.close();
				
				System.out.println("CTime:" + Data27Bean.getCTime());
				System.out.println("Order_Id:" + Data27Bean.getOrder_Id());
				System.out.println("Order_Value:" + new BigDecimal(Data27Bean.getOrder_Value()).divide(new BigDecimal(100),2,java.math.RoundingMode.HALF_UP));
				System.out.println("Value:" + new BigDecimal(Data27Bean.getValue()).divide(new BigDecimal(100),2,java.math.RoundingMode.HALF_UP));
				System.out.println("Car_Id:" + Data27Bean.getCar_Id());
				System.out.println("Car_Owner:" + Data27Bean.getCar_Owner());
				System.out.println("Worker:" + Data27Bean.getWorker());
				System.out.println("Oil_CType:" +  Data27Bean.getOil_CType());
				
				String Sql = "insert into pro_i(cpm_id, ctime, order_id, order_value, value, oil_ctype, car_id, car_owner, worker)" +
		  	  	  			 "values('"+ this.getActionSource().trim() +"', " +
		  	  	  			 "date_format('"+ Data27Bean.getCTime().trim() +"', '%Y-%m-%d %H-%i-%S'), " +
		  	  	  			 "'"+ Data27Bean.getOrder_Id().trim() +"', " +
			  	  	     	 "'"+ new BigDecimal(Data27Bean.getOrder_Value()).divide(new BigDecimal(100),2,java.math.RoundingMode.HALF_UP) +"', " +
			  	  	     	 "'"+ new BigDecimal(Data27Bean.getValue()).divide(new BigDecimal(100),2,java.math.RoundingMode.HALF_UP) +"', " +
			  	  	     	 "'"+ Data27Bean.getOil_CType().trim() +"', " +			  	  	     	 
			  	  	     	 "'"+ Data27Bean.getCar_Id().trim() +"', " +		  
			  	  	     	 "'"+ Data27Bean.getCar_Owner().trim() +"', " +
			  	  	     	 "'"+ Data27Bean.getWorker().trim() +"')";
