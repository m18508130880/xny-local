...
<form name="Pro_L"  action="Pro_L.do" method="post" target="mFrame">
<div id="down_bg_2">
	<table width="100%" style='margin:auto;' border=0 cellPadding=0 cellSpacing=0 bordercolor="#3491D6" borderColorDark="#ffffff">
        ...
        for(int k=1; k<13; k++)
        {
       	...
       	%>
			...
			<td width='30%' align='right'>		
				...
				<!--<img id="img3" src="../skin/images/tubiaofenxi.gif"        onClick='doGraph()'>-->
			</td>
		</tr>
		<tr height='30' valign='middle'>
			<td width='100%' align='center' colspan=2>
				...
				if(null != Pro_L)
				{
					Iterator iterator = Pro_L.iterator();
					while(iterator.hasNext())
					{
						...
						if(!T_Cpm_Id.equals(Bean.getCpm_Id()))
						{
							if(T_Cpm_Id.length() > 0)
							{
								...
								if(null != Pro_Y)
								{
									Iterator yeariter = Pro_Y.iterator();
									while(yeariter.hasNext())
									{
										ProLBean yearBean = (ProLBean)yeariter.next();
										if(T_Cpm_Id.equals(yearBean.getCpm_Id()))
										{
				...
				%>
								<tr height='30'>
								  <td width='16%' align=center><strong>本月累计</strong></td>
								  <td width='14%' align=center><strong><%=new BigDecimal(Value_O_All).divide(new BigDecimal(1),2,java.math.RoundingMode.HALF_UP)%></strong></td>
								  <td width='14%' align=center><strong><%=new BigDecimal(Value_O_Gas_All).divide(new BigDecimal(1),2,java.math.RoundingMode.HALF_UP)%></strong></td>
								  <td width='14%' align=center><strong><%=new BigDecimal(Value_I_All).divide(new BigDecimal(1),2,java.math.RoundingMode.HALF_UP)%></strong></td>
								  <td width='14%' align=center><strong><%=new BigDecimal(Value_I_Gas_All).divide(new BigDecimal(1),2,java.math.RoundingMode.HALF_UP)%></strong></td>
								  <td width='14%' align=center><strong>&nbsp;</strong></td>
								  <td width='14%' align=center><strong>&nbsp;</strong></td>
								</tr>
								<tr height='30'>
								  <td width='16%' align=center><strong>本年累计</strong></td>
								  <td width='14%' align=center><strong><%=Value_O_Y%></strong></td>
								  <td width='14%' align=center><strong><%=Value_O_Gas_Y%></strong></td>
								  <td width='14%' align=center><strong><%=Value_I_Y%></strong></td>
								  <td width='14%' align=center><strong><%=Value_I_Gas_Y%></strong></td>
								  <td width='14%' align=center><strong>&nbsp;</strong></td>
								  <td width='14%' align=center><strong>&nbsp;</strong></td>
								</tr>
								<tr height='30'>
								  <td width='30%' align=center colspan=2><strong>制表: </strong>系统</td>
								  <td width='28%' align=center colspan=2><strong>审核: </strong><%=Operator_Name%></td>
								  <td width='42%' align=center colspan=3><strong>上报日期: </strong><%=CommUtil.getDate()%></td>
								</tr>
				<%
								Value_O_All = 0;
								Value_O_Gas_All = 0;
				...
				%>
							<tr height='50'>
								<!--<td width='100%' align=center colspan=7><font size=4><B>加油站/加气站生产运营月报表[<%=Oil_Name%>]</B></font></td>-->
							<td width='100%' align=center colspan=7><font size=4><B>加气站生产运营月报表</B></font></td>
							</tr>
							<tr height='30'>
								<td width='100%' align=center colspan=7>
									<table width="100%" border=0 cellPadding=0 cellSpacing=0 bordercolor="#3491D6" borderColorDark="#ffffff">
										<tr height='30'>
											...
											<td width='70%' align=left><strong>站点名称:<%=T_Cpm_Name%>站</strong></td>
							...
							<tr height='30'>
						    <td width='16%' align=center rowspan='2'><strong>日期\项目</strong></td>
						    <td width='28%' align=center colspan='2'><strong>销售数量</strong></td>
						    <td width='28%' align=center colspan='2'><strong>购入数量</strong></td>
						    <td width='28%' align=center colspan='2'><strong>库存数量</strong></td>
						  </tr>
						  <tr height='30'>
						  	<%
						  	String Name1 = "";
							...
							%>
											<td width='14%' align=center><%=Name1%></td>
									    <td width='14%' align=center><%=Name2%></td>
									    <td width='14%' align=center><%=Name1%></td>
									    <td width='14%' align=center><%=Name2%></td>
									    <td width='14%' align=center><%=Name1%></td>
									    <td width='14%' align=center><%=Name2%></td>
						  ...
						  <tr height='30'>
						    <td width='16%' align=center><%=Integer.parseInt(CTime.substring(8,10))%></td>
						    <td width='14%' align=center><%=Value_O%></td>
						    <td width='14%' align=center><%=Value_O_Gas%></td>
						    <td width='14%' align=center><%=Value_I%></td>
						    <td width='14%' align=center><%=Value_I_Gas%></td>
						    <td width='14%' align=center><%=Value_R%></td>
						    <td width='14%' align=center><%=Value_R_Gas%></td>
						  </tr>
				<%
						}
						else
...
//window.parent.frames.lFrame.document.getElementById('CurrJsp').innerText = 'Pro_L.jsp';
	...
	switch(parseInt(pFunc_Sub_Id))
	{
		case 1://上月
			  ...
			  var Month = BTime.substring(5,7);
				window.parent.frames.mFrame.location = "Pro_L.do?Cmd=0&Sid=<%=Sid%>&Cpm_Id="+Pro_L.Func_Cpm_Id.value+"&Func_Sub_Id="+pFunc_Sub_Id+"&Func_Corp_Id="+Pro_L.Func_Corp_Id.value+"&BTime="+BTime+"&ETime="+ETime+"&Year="+Year+"&Month="+Month;
			break;
		case 2://本周
				...
				var BTime = TDay;
				var ETime = TDay;
				window.parent.frames.mFrame.location = "Pro_L.do?Cmd=3&Sid=<%=Sid%>&Cpm_Id="+Pro_L.Func_Cpm_Id.value+"&Func_Sub_Id="+pFunc_Sub_Id+"&Func_Corp_Id="+Pro_L.Func_Corp_Id.value+"&BTime="+BTime+"&ETime="+ETime+"&Year="+Year+"&Month="+Month+"&Week="+Week;
			break;
		case 3://昨天
				var BTime = showPreviousDay().format("yyyy-MM-dd");
				window.parent.frames.mFrame.location = "Pro_L.do?Cmd=4&Sid=<%=Sid%>&Cpm_Id="+Pro_L.Func_Cpm_Id.value+"&Func_Sub_Id="+pFunc_Sub_Id+"&Func_Corp_Id="+Pro_L.Func_Corp_Id.value+"&BTime="+BTime;
			break;
		case 4: //年
		window.parent.frames.mFrame.location = "Pro_Y.do?Cmd=5&Sid=<%=Sid%>&Cpm_Id=0100000001&Func_Sub_Id="+pFunc_Sub_Id+"&Func_Corp_Id="+Pro_L.Func_Corp_Id.value+"&BTime="+BTime+"&Year="+Pro_L.Year.value;
			break;
	}
}
...
function doSelect()
{
	Pro_L.Cpm_Id.value = Pro_L.Func_Cpm_Id.value;
	Pro_L.BTime.value = Pro_L.Year.value + '-' + StrLeftFillZero(Pro_L.Month.value,2) + '-01';
  Pro_L.ETime.value = getFirstAndLastMonthDay(Pro_L.Year.value, StrLeftFillZero(Pro_L.Month.value,2));
	Pro_L.submit();
}
...
function doExport()
{
	...
	var BTime = Pro_L.Year.value + '-' + StrLeftFillZero(Pro_L.Month.value,2) + '-01';
	var ETime = getFirstAndLastMonthDay(Pro_L.Year.value, StrLeftFillZero(Pro_L.Month.value,2));
	if(confirm("确定导出?"))
  {
		if(window.XMLHttpRequest)
	  {
		...
		var url = "Pro_L_Export.do?Sid=<%=Sid%>&Operator_Name=<%=Operator_Name%>&Cpm_Id="+Pro_L.Func_Cpm_Id.value+"&BTime="+BTime+"&ETime="+ETime+"&Func_Sub_Id="+Pro_L.Func_Sub_Id.value+"&Func_Corp_Id="+Pro_L.Func_Corp_Id.value+"&Year="+Pro_L.Year.value+"&Month="+Pro_L.Month.value+"&Cpm_Name=<%=T_Cpm_Name%>";
...
function doGraph()
{
  ...
  window.parent.frames.mFrame.location = "Pro_G.do?Cmd=20&Sid=<%=Sid%>&Cpm_Id="+Pro_L.Func_Cpm_Id.value+"&Func_Corp_Id="+Pro_L.Func_Corp_Id.value+"&Func_Sub_Id=1&Func_Sel_Id=1&BYear="+BYear+"&BMonth="+BMonth+"&EYear="+EYear+"&EMonth="+EMonth;
}
</SCRIPT>
</html>