		...
		<tr height='30' valign='middle'>
			<td width='100%' align='center' colspan=2>
				...
				%>
					<tr height='50'>
						<td width='100%' align=center colspan=12>
							...
							<font size=4><B>LNG汽车加气站生产运营日报表</B></font>
						</td>
					</tr>
					<tr height='30'>
						<td width='100%' colspan=12>				
							<table width="100%" border=0 cellPadding=0 cellSpacing=0 bordercolor="#3491D6" borderColorDark="#ffffff">
										<tr height='30'>
											...
											<td width='25%' align=left><strong>数据日期: <%=BDate%></strong></td>
										</tr>
									</table>															
						</td>
					</tr>
					<tr height='30'>
				    <td width='10%' align=center rowspan="2"><strong>站名称\项目</strong></td>
				    <td width='18%' align=center colspan="2"><strong>销售数量</strong></td>
				    <td width='18%' align=center colspan="2"><strong>购入数量</strong></td>
				    <td width='18%' align=center colspan="2"><strong>库存数量</strong></td>
				    <td width='18%' align=center colspan="2"><strong>盈亏数量</strong></td>
				    <td width='18%' align=center  rowspan="2" colspan="2"><strong>退回报表</strong></td>
				  </tr>
				  <tr height='30'>
				  	<%
				  	String Name1 = "";
						  	...
						  	switch(Integer.parseInt(currStatus.getFunc_Corp_Id()))
								...
								case 3002://LNG
									...
									<td width='10%' align=center><strong><%= Name1%></strong></td>
							    <td width='10%' align=center><strong><%= Name2%></strong></td>
							    <td width='10%' align=center><strong><%= Name1%></strong></td>
							    <td width='10%' align=center><strong><%= Name2%></strong></td>
							    <td width='10%' align=center><strong><%= Name1%></strong></td>
							    <td width='10%' align=center><strong><%= Name2%></strong></td>
				...
				%>
					    ...
					    <td width='10%' align=center><%=Value_PAL%></td>
					    <td width='10%' align=center><%=Value_PAL_Gas%></td>
					    <td  align=center colspan="2"><a href="#" onClick="doPro_L_Del('<%=Cpm_Id%>','<%=CTime%>')"><font color="red"><U>退回</U></font></a></td>
						</tr>
				<%
					}
				%>
				    ...
				    <td width='10%' align=center><strong><%=new BigDecimal(Value_PAL_All).divide(new BigDecimal(1),2,java.math.RoundingMode.HALF_UP)%></strong></td>
				    <td width='10%' align=center><strong><%=new BigDecimal(Value_PAL_Gas_All).divide(new BigDecimal(1),2,java.math.RoundingMode.HALF_UP)%></strong></td>
				 		<td width='16%' align=center colspan="2"><strong></strong></td>
				  </tr>		
				  <tr height='80'>
				    <td width='10%' align=center><strong>备注</strong></td>
				    <td width='90%' align=left colspan=10 valign=top>
				    	1）本日加注车次：
		<%
				if( null != Pro_L_Crp_D_Crm && !Pro_L_Crp_D_Crm.isEmpty())
						{
			...
			%>
								<%=crmBean.getCrm_Name()%>加注:&nbsp;<%=crmBean.getValue_I_Cnt()%>&nbsp;辆车次;
			<%					
									Value_I_Cnt =  Integer.parseInt(crmBean.getValue_I_Cnt());
			...
			%>	    	
							共计加注车次：<%=Car_Cnt_All%> 辆次。
				    	<br>		    	
				    	2）今日盘亏：<%=new BigDecimal(Value_PAL_All).divide(new BigDecimal(1),2,java.math.RoundingMode.HALF_UP)%>(kg)。
				    	<br>
				    	3）购入气源：<%=new BigDecimal(Value_I_All).divide(new BigDecimal(1),2,java.math.RoundingMode.HALF_UP)%>(kg)。
				    </td>
				  </tr>
				  <tr height='30'>
				    <td width='30%' align=center colspan=5><strong>制表: </strong>系统</td>
				    <td width='30%' align=center colspan=6><strong>审核: </strong><%=Operator_Name%></td>
...
function doChangeSelect(pFunc_Sub_Id)
{
	switch(parseInt(pFunc_Sub_Id))
	{
		case 0://今年
				...
				window.parent.frames.mFrame.location = "Pro_L_Crp.do?Cmd=0&Sid=<%=Sid%>&Cpm_Id="+Pro_L_Crp_D.Func_Cpm_Id.value+"&Func_Sub_Id="+pFunc_Sub_Id+"&Func_Corp_Id="+Pro_L_Crp_D.Func_Corp_Id.value+"&BTime="+BTime+"&ETime="+ETime+"&Year="+Year;
			break;
		case 1://上月
			  ...
			  var Month = BTime.substring(5,7);
				window.parent.frames.mFrame.location = "Pro_L_Crp.do?Cmd=1&Sid=<%=Sid%>&Cpm_Id="+Pro_L_Crp_D.Func_Cpm_Id.value+"&Func_Sub_Id="+pFunc_Sub_Id+"&Func_Corp_Id="+Pro_L_Crp_D.Func_Corp_Id.value+"&BTime="+BTime+"&ETime="+ETime+"&Year="+Year+"&Month="+Month;
			break;
		case 2://本周
				...
				var BTime = TDay;
				var ETime = TDay;
				window.parent.frames.mFrame.location = "Pro_L_Crp.do?Cmd=2&Sid=<%=Sid%>&Cpm_Id="+Pro_L_Crp_D.Func_Cpm_Id.value+"&Func_Sub_Id="+pFunc_Sub_Id+"&Func_Corp_Id="+Pro_L_Crp_D.Func_Corp_Id.value+"&BTime="+BTime+"&ETime="+ETime+"&Year="+Year+"&Month="+Month+"&Week="+Week;
			break;
		case 3://昨天
				var BTime = showPreviousDay().format("yyyy-MM-dd");
				window.parent.frames.mFrame.location = "Pro_L_Crp.do?Cmd=3&Sid=<%=Sid%>&Cpm_Id="+Pro_L_Crp_D.Func_Cpm_Id.value+"&Func_Sub_Id="+pFunc_Sub_Id+"&Func_Corp_Id="+Pro_L_Crp_D.Func_Corp_Id.value+"&BTime="+BTime;
			break;
	}
}
...
function doSelect()
{
	Pro_L_Crp_D.Cpm_Id.value = Pro_L_Crp_D.Func_Cpm_Id.value;
...
function doExport()
{
	...
	if(confirm("确定导出?"))
  {
		if(window.XMLHttpRequest)
	  {
		...
		var url = "Pro_L_Crp_D_Export.do?Sid=<%=Sid%>&Operator_Name=<%=Operator_Name%>&Cpm_Id="+Pro_L_Crp_D.Func_Cpm_Id.value+"&BTime="+BTime+"&Func_Sub_Id="+Pro_L_Crp_D.Func_Sub_Id.value+"&Func_Corp_Id="+Pro_L_Crp_D.Func_Corp_Id.value;
...
function doGraph()
{
  ...
  window.parent.frames.mFrame.location = "Pro_Crp_G.do?Cmd=20&Sid=<%=Sid%>&Cpm_Id="+Pro_L_Crp_D.Func_Cpm_Id.value+"&Func_Corp_Id=3001&Func_Sub_Id=1&Func_Sel_Id=1&BYear="+BYear+"&BMonth="+BMonth;
}
function doPro_L_Del(pCpm_Id, pCTime)
{		
	if(confirm("是否退掉本条记录?"))
		{
			location="Pro_L_Crp.do?Cmd=11&Sid=<%=Sid%>&Cpm_Id="+Pro_L_Crp_D.Func_Cpm_Id.value+"&Func_Sub_Id="+Pro_L_Crp_D.Func_Sub_Id.value+"&Func_Corp_Id="+Pro_L_Crp_D.Func_Corp_Id.value+"&BTime="+pCTime+"&BCpm_Id="+pCpm_Id;
		}
}
